...
We are adding a `--sessions` mode to the `/reflect` command in tailored-omo. This mode enables cross-session reflection by analyzing past OpenCode sessions to find repeated patterns, friction, and improvement opportunities.
...
**Decision:** Store reflection summaries in `~/.config/opencode/tailored-omo/reflections/`.
...
**Structure:**
```
~/.config/opencode/tailored-omo/reflections/
  sessions/
...
|--------|------|------|----------|
| `~/.config/opencode/tailored-omo/reflections/` | Existing directory, single location | Mixes config and data | **Accepted** |
| `~/.local/share/tailored-omo/reflections/` | XDG-compliant, data separation | New directory, splits OMOS data | Rejected |
