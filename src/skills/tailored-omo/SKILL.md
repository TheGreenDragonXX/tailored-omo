---
name: tailored-omo
description: Configure and improve tailored-omo for the current user. Use when users want to tune agents, models, prompts, custom agents, skills, MCPs, presets, or plugin behavior. Also use when recurring workflow friction suggests a safe config or prompt improvement.
---

# tailored-omo Configuration Skill
...
- `~/.config/opencode/tailored-omo.json` or `.jsonc`
...
tailored-omo is configured through a plugin config file, usually:

```text
~/.config/opencode/tailored-omo.json
```

or:

```text
~/.config/opencode/tailored-omo.jsonc
```
...
| `~/.config/opencode/tailored-omo.jsonc` | User plugin config with comments/trailing commas; takes precedence over `.json` |
| `~/.config/opencode/tailored-omo.json` | User plugin config generated by installer |
| `<project>/.opencode/tailored-omo.json` | Project-local plugin overrides checked before user config |
| `~/.config/opencode/tailored-omo/{agent}.md` | Full prompt replacement for a built-in agent |
| `~/.config/opencode/tailored-omo/{agent}_append.md` | Append-only prompt tuning for a built-in agent |
| `~/.config/opencode/tailored-omo/{preset}/{agent}.md` | Preset-specific full prompt replacement |
| `~/.config/opencode/tailored-omo/{preset}/{agent}_append.md` | Preset-specific append-only prompt tuning |
...
1. If a `preset` is active, check
   `~/.config/opencode/tailored-omo/{preset}/` first.
2. Fall back to `~/.config/opencode/tailored-omo/`.
...
- Built-in agents (`orchestrator`, `oracle`, `librarian`, `explorer`, `fixer`,
  `observer`) can set models, variants, skills,
  MCPs, options, and display names in config.
- Built-in agent `prompt` and `orchestratorPrompt` fields are **not** supported
  in `tailored-omo.json[c]`; use markdown prompt override files instead.
...
```text
~/.config/opencode/tailored-omo/orchestrator_append.md
```

or for only the `openai` preset:

```text
~/.config/opencode/tailored-omo/openai/orchestrator_append.md
```
...
```text
~/.config/opencode/tailored-omo/orchestrator.md
```
...
1. **Inspect current setup**
   - Read the existing tailored-omo config file.
...
~/.config/opencode/tailored-omo/orchestrator_append.md. Apply it?
```
...
public API changes to it. Apply to your tailored-omo config?
```
