...
describe('loadPluginConfig', () => {
  ...
  test('loads project config from .opencode directory', () => {
    ...
    fs.writeFileSync(
      path.join(projectConfigDir, 'tailored-omo.json'),
      JSON.stringify({
        agents: {
  ...
  test('validates auto image routing after project enables Observer', () => {
    ...
    fs.mkdirSync(projectConfigDir, { recursive: true });
    fs.writeFileSync(
      path.join(userConfigPath, 'tailored-omo.json'),
  ...
  test('respects OPENCODE_CONFIG_DIR for user config location', () => {
    ...
    fs.writeFileSync(
      path.join(customDir, 'tailored-omo.json'),
      JSON.stringify({
  ...
  test('falls back to default user config dir when OPENCODE_CONFIG_DIR has no config', () => {
    ...
    fs.writeFileSync(
      path.join(defaultConfigDir, 'tailored-omo.json'),
      JSON.stringify({
...
describe('onWarning callback', () => {
  ...
  test('read error calls onWarning with read-error', () => {
    const projectDir = path.join(tempDir, 'project');
    const projectConfigDir = path.join(projectDir, '.opencode');
    const configPath = path.join(projectConfigDir, 'tailored-omo.json');
    fs.mkdirSync(projectConfigDir, { recursive: true });
...
describe('deepMerge behavior', () => {
  ...
  test('merges nested agent configs from user and project', () => {
    ...
    fs.writeFileSync(
      path.join(userOpencodeDir, 'tailored-omo.json'),
      JSON.stringify({
        agents: {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        agents: {
          ...
          reviewer: { model: 'project/reviewer-model' }, // Add new agent
        },
      }),
    );
    ...
    // reviewer: from project only
    expect(config.agents?.reviewer?.model).toBe('project/reviewer-model');
  });
...
describe('environment variable preset override', () => {
  ...
  test('Env var overrides preset from config file', () => {
    ...
    process.env.TAILORED_OMO_PRESET = 'env-preset';
    const config = loadPluginConfig(projectDir);
  ...
  test('Env var is ignored if empty string', () => {
    ...
    process.env.TAILORED_OMO_PRESET = '';
    const config = loadPluginConfig(projectDir);
  ...
  test('Env var with nonexistent preset warns and falls back', () => {
    ...
    process.env.TAILORED_OMO_PRESET = 'typo-preset';
    const consoleWarnSpy = spyOn(console, 'warn');
    const config = loadPluginConfig(projectDir);
...
describe('JSONC config support', () => {
  ...
  test('loads .jsonc file with single-line comments', () => {
    ...
    fs.mkdirSync(projectConfigDir, { recursive: true });
    fs.writeFileSync(
      path.join(projectConfigDir, 'tailored-omo.jsonc'),
      `{
        // This is a comment
        "agents": {
  ...
  test('loads user config from .jsonc', () => {
    ...
    fs.mkdirSync(userOpencodeDir, { recursive: true });
    fs.writeFileSync(
      path.join(userOpencodeDir, 'tailored-omo.jsonc'),
      `{
...
describe('loadAgentPrompt', () => {
  ...
  test('loads replacement prompt from {agent}.md', () => {
    const promptsDir = path.join(tempDir, 'opencode', 'tailored-omo');
  ...
  test('works with XDG_CONFIG_HOME environment variable', () => {
    ...
    const promptsDir = path.join(customConfigHome, 'opencode', 'tailored-omo');
    fs.mkdirSync(promptsDir, { recursive: true });
  ...
  test('respects OPENCODE_CONFIG_DIR for prompt location', () => {
    ...
    const promptsDir = path.join(customDir, 'tailored-omo');
    fs.mkdirSync(promptsDir, { recursive: true });
    fs.writeFileSync(
