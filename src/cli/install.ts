...
const GITHUB_REPO = 'TheGreenDragonXX/tailored-omo';
const GITHUB_URL = `https://github.com/${GITHUB_REPO}`;

function printHeader(isUpdate: boolean): void {
  console.log();
  console.log(`${BOLD}tailored-omo ${isUpdate ? 'Update' : 'Install'}${RESET}`);
...
async function runInstall(config: InstallConfig): Promise<number> {
  ...
  printStep(step++, totalSteps, 'Adding tailored-omo plugin...');
  if (config.dryRun) {
  ...
  printStep(step++, totalSteps, 'Writing tailored-omo configuration...');
  if (config.dryRun) {
  ...
  const docsUrl =
    'https://github.com/TheGreenDragonXX/tailored-omo/' +
    'blob/tailored-main/docs/configuration.md';
