...
describe('config-io', () => {
  ...
  test('addPluginToOpenCodeConfig adds plugin and removes duplicates', async () => {
    ...
    writeFileSync(
      configPath,
      JSON.stringify({ plugin: ['other', 'tailored-omo@1.0.0'] }),
    );
    process.argv[1] = '';
    ...
    expect(saved.plugin).toContain('tailored-omo');
    expect(saved.plugin).not.toContain('tailored-omo@1.0.0');
  ...
  test('addPluginToOpenCodeConfig respects OPENCODE_CONFIG_DIR', async () => {
    ...
    expect(customSaved.plugin).toEqual(['other', 'tailored-omo']);
  ...
  test('addPluginToOpenCodeConfig stores package name for bunx temp paths', async () => {
    ...
    expect(result.success).toBe(true);
    const saved = JSON.parse(readFileSync(configPath, 'utf-8'));
    expect(saved.plugin).toEqual(['tailored-omo']);
  });
  ...
  test('addPluginToOpenCodeConfig preserves non-string plugin entries when refreshing', async () => {
    ...
    writeFileSync(
      configPath,
      JSON.stringify({
        plugin: ['other-plugin', objectPlugin, 'tailored-omo@1.0.0'],
      }),
    );
  ...
  test('addPluginToOpenCodeConfig removes tuple plugin entries', async () => {
    ...
    writeFileSync(
      configPath,
      JSON.stringify({
        plugin: ['other', ['tailored-omo', { enabled: true }]],
      }),
    );
    process.argv[1] = '';
    ...
    const saved = JSON.parse(readFileSync(configPath, 'utf-8'));
    expect(saved.plugin).toEqual(['other', 'tailored-omo']);
  });
  ...
  test('addPluginToOpenCodeTuiConfig does not bypass OPENCODE_TUI_CONFIG for existing default config', async () => {
    ...
    expect(custom.plugin).toEqual(['tailored-omo']);
  ...
  test('writeLiteConfig writes lite config with OpenAI preset', () => {
    const litePath = join(tmpDir, 'opencode', 'tailored-omo.json');
    paths.ensureConfigDir();
  ...
  test('detectCurrentConfig detects provider models in arrays', () => {
    ...
    writeFileSync(configPath, JSON.stringify({ plugin: ['tailored-omo'] }));
    writeFileSync(
      litePath,
      JSON.stringify({
        preset: 'dev',
